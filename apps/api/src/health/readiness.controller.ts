 ...
 * want. Architecture section 15.6 assumes processes are replaceable and behind an edge, so both
 ...
 * the deny by default guard required by architecture section 6.2 lands, both health routes must
...
export class ReadinessController {
  ...
  async check(): Promise<ReadinessResponse> {
    try {
      ...
      // The message is deliberately generic. Architecture section 2.10 forbids an error from
